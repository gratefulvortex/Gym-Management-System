...
def renew_membership(request, member_id):
    ...
    if member.end_date < current_date:
        new_end_date = current_date + timedelta(days=member.tenure * 30)  # Assuming tenure is in months
        member.end_date = new_end_date
        member.start_date = current_date  # Update start date to current date
        member.save()
        messages.success(request, f'Membership for {member.name} {member.last_name} has been renewed until {new_end_date}.')
    else:
        messages.error(request, 'Membership is still active and cannot be renewed.')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            member = Member.objects.create(
                ...
                last_name=membership.last_name,  # Add last name here
